...
export default function TProfile(props) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      try {
        ...
        setTimeout(() => {
          axios
            .get(`http://localhost:5555/ssearch/${decodedToken.rno}`)
            .then((res) => {
              if (res.data.message === "Found") {
                const rno = res.data.data[0].rno;
                ...
                setFormData({ rno, name, email, password, contact });
                setLoading(false);
              } else {
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .put("http://localhost:5555/sedit", formData)
      .then((res) => {
        ...
        <div className="glass p-4 dashboard-panel">
          ...
          <form className="row g-4" onSubmit={handleSubmit}>
            <div className="col-md-6">
              <label className="form-label">Student ID (rno)</label>
              <input
                type="text"
